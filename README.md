...
Godot 4.6 东方 Project 风格 Boss 战 / 弹幕射击游戏原型。当前以阿米娅（Amiya）Boss 战为测试场景，包含弹幕对象池、Boss 状态机、符卡机制、掉落物框架、玩家受击 / 擦弹、演出管理、调试控制台和 3 级武器系统。
...
如果子弹在物理回调中被命中或清屏回收，`BulletManager.recycle()` 会延迟到安全时机执行真正的 `bullet_off()`，避免 Godot 4 在 physics callback 中禁用碰撞对象时报错。
...
### UI 系统
`UIManager` 提供 68/32 左右分屏布局：左侧游戏区域，右侧信息栏（分数 / 擦弹分 / 残机 / Bomb / Power）。玩家 life 归零后会在左侧游戏区域显示 `GAME OVER` 覆盖层和 `RESTART` 按钮；按钮发出 `restart` 信号，由 `GameManager` 执行开发期重置流程。

### 玩家受击 / Bomb / Game Over
`player.gd` 负责自机受击判定、deathbomb 窗口、死亡、复活和 Game Over 入口。玩家判定点接触敌人或敌方子弹后会进入短暂的被弹判定时间；若此期间按下 X 并成功使用 Bomb，则清除敌方子弹并取消本次死亡。普通死亡会扣除 1 点 life、清除敌方子弹，并在短暂延迟后复活到左侧游戏区域下方中央，随后进入 3 秒无敌状态。

当 life 扣到 0 时，玩家不再复活，`GameManager.game_over()` 显示 Game Over UI。开发期 `restart` 会重置 life、bomb、power、Boss 阶段、Boss 剩余符卡序列、玩家火力等级、玩家位置与当前关卡进度。

### 玩家擦弹
`player.gd` 通过 `GrazeArea` 监听敌方子弹进入擦弹范围。敌方子弹第一次进入时会触发一次擦弹判定，调用 `UIManager.add_graze_score()` 增加独立擦弹分；擦弹分显示在右侧信息栏 `TOTAL SCORE` 下一行，不计入总分。

擦弹去重标记保存在子弹 metadata 中，避免同一颗子弹反复进出擦弹范围时重复加分。对象池子弹在 `bullet_on()` 重新激活时会清除该标记，因此复用后的新一轮发射仍可再次产生擦弹。
...
### 关卡进度
`LevelManager` 目前是轻量进度节点，挂在 `game.tscn` 的 `LevelManager` 节点上。当前只提供 `current_progress` / `default_progress` 和 `reset_progress()`，用于 Restart 时清理开发期关卡进度；后续正式关卡流程可以继续扩展这个节点。
